...
extension RangeReplaceableCollection {
    ...
    public mutating func removeDuplicates<E: Equatable>(keyPath path: KeyPath<Element, E>) {
        var items = [Element]()
    ...
    public mutating func removeDuplicates<E: Hashable>(keyPath path: KeyPath<Element, E>) {
        var set = Set<E>()
    ...
    public subscript<R>(range: R) -> SubSequence where R: RangeExpression, R.Bound == Int {
        get {
        ...
        set {
            let indexRange = range.relative(to: 0..<count)
            replaceSubrange(index(startIndex, offsetBy: indexRange.lowerBound)..<index(startIndex, offsetBy: indexRange.upperBound),
                            with: newValue)
        }
    }
     ...
     */
    public mutating func appendIfNonNil(_ newElement: Element?) {
     ...
     */
    public mutating func appendIfNonNil<S>(contentsOf newElements: S?) where Element == S.Element, S: Sequence {
